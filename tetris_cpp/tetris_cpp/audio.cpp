...
AudioManager::AudioManager()
    : stream(nullptr), wav_data(nullptr), wav_data_len(0) {
}
...
std::string AudioManager::getExecutablePath(const char* filename) {
...
bool AudioManager::init() {
    ...
    SDL_AudioSpec spec;
    std::string wavPath = getExecutablePath("TetrisBg.wav");

    if (!SDL_LoadWAV(wavPath.c_str(), &spec, &wav_data, &wav_data_len)) {
...
void AudioManager::play() {
    if (stream && wav_data && wav_data_len > 0) {
